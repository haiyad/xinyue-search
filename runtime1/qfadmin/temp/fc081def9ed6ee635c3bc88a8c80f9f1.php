...
<?php endif; ?>
    ...
    <el-table :data="dataList" @selection-change="changeSelection" v-loading="loading">
        ...
        <el-table-column label="分类名称">
            <template slot-scope="scope">
                <img :src="scope.row.image" style="width: 30px;height:30px;vertical-align: middle;"
                    v-if="scope.row.image" />
                <span style="vertical-align: middle;margin-left: 5px">{{scope.row.name}}</span>
            </template>
        </el-table-column>
        <el-table-column label="是否展示本地" width="150" align="center">
            <template slot-scope="scope">
                <el-switch v-model="scope.row.is_type==1?true:false"
                    @change="clickStatus(scope.row.source_category_id,'is_type',scope.row.is_type)">
                </el-switch>
            </template>
        </el-table-column>
        <el-table-column label="是否每日更新" width="150" align="center">
            <template slot-scope="scope">
                <el-switch v-model="scope.row.is_update==1?true:false"
                    @change="clickStatus(scope.row.source_category_id,'is_update',scope.row.is_update)">
        ...
        <el-table-column label="是否前台展示" width="150" align="center">
            <template slot-scope="scope">
                <el-switch v-model="scope.row.status==1?false:true"
                    @change="clickStatus(scope.row.source_category_id,'status',scope.row.status)">
                </el-switch>
            </template>
        </el-table-column>
    ...
    <!-- 添加框 -->
    <el-dialog title="添加资源分类名称" :visible.sync="dialogFormAdd" width="500px" :modal-append-to-body='false' append-to-body
    ...
    <!-- 修改框 -->
    <el-dialog title="修改资源分类信息" :visible.sync="dialogFormEdit" width="500px" :modal-append-to-body='false'
    ...
    <script>
        var app = new Vue({
            ...
            methods: {
                ...
                clickStatus(source_category_id, type, status) {
                    var that = this;
